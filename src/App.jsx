...
function Showreel() {
  ...
  const seek = (offset) => {
    if (videoRef.current) {
      ...
      const target = Math.max(0, Math.min(video.duration || 0, video.currentTime + offset));
    ...
    <section className="showreel-section" ref={ref}>
      ...
      <div className={`showreel-video-wrap ${isPlaying ? "active" : ""}`}>
        ...
        <div className="showreel-controls">
          <button type="button" className="showreel-control showreel-control--prev" onClick={() => seek(-10)} aria-label="Rewind 10 seconds">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="11 18 5 12 11 6"/><path d="M19 18V6"/></svg>
          </button>
          <button type="button" className="showreel-control showreel-control--next" onClick={() => seek(10)} aria-label="Forward 10 seconds">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="13 18 19 12 13 6"/><path d="M5 18V6"/></svg>
          </button>
        </div>
      </div>
    </section>
    ...
    <>
      ...
      <style>{`
        ...
        * {
        ...
        .showreel-control svg {
          stroke: #fff;
        }

        .showreel-control--prev {
          margin-left: 5px;
        }

        .showreel-control--next {
          margin-right: 5px;
        }
